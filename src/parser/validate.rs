...
pub fn validate_block(
	...
	for statement in statements {
		validate_statement(statement, tree, &mut variables, return_type)?;
	}
	...
	match &statement.kind {
		...
		StatementKind::If {
			...
			if let Some(else_statement) = else_statement {
				if let StatementKind::If { .. } = &else_statement.kind {
					validate_statement(else_statement, tree, variables, return_type)?;
				} else if let StatementKind::Block { statements } = &else_statement.kind {
					validate_block(statements, tree, variables.clone(), return_type)?;
				}
			}
		}
